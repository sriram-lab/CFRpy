function [fluxstate, grate, solverobj, solg1, model] = constrain_flux_regulation(...
    ...
    kappa, rho, epsilon, mode, epsilon2, minfluxflag, kappa2, params)
    ...
    % Updates:  July 14, 2026 (Carolina H. Chung)
    %           September 20, 2020 (Carolina H. Chung)
    ...
    OPTIONAL INPUTS:
        1. rho:         relative weight for "on" reactions/up-reg genes
           -> default:  rho = 1
           -> note:     use small value (e.g. 1e-3) for large networks
    
        2. kappa:       relative weight for "off" reactions/down-reg genes
           -> default:  kappa = 1
           -> note:     use small value (e.g. 1e-3) for large networks
        ...
        7. kappa2:      minimum flux for pFBA reactions 
           -> default:  kappa2 = 1e-6

        8. params:      parameter values to pass to gurobi 
           -> default:  params.OutputFlag = 0
...
%% INSPECT INPUTS AND SET DEFAULT VALUES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Consolidate contradicting reactions
    if any(ismember(onreactions, offreactions))
        offreactions = setdiff(offreactions, onreactions);
    end
    onreactions = sort(onreactions);
    offreactions = sort(offreactions);
    ...
    if minfluxflag
        kappa = [kappa(:); ...
            ones(size(setdiff(model1.rxns, offreactions))) * kappa2]; 
        epsilon2 = [epsilon2; ...
